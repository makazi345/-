--==========================================================
--   MAKAZI MOD  •  v10.0  •  FULL MOBILE EDITION
--==========================================================
print("=== MAKAZI MOD v10 START ===")
...
pcall(function()
	CoreGui:FindFirstChild("MakaziMod"):Destroy()
end)
pcall(function()
	playerGui:FindFirstChild("MakaziMod"):Destroy()
end)
...
local function clickSound()
	playSound(SOUND_IDS.click, 0.35, 1)
end
local function toggleOn()
	playSound(SOUND_IDS.click, 0.4, 1.3)
end
local function toggleOff()
	playSound(SOUND_IDS.click, 0.4, 0.85)
end
local function openSound()
	playSound(SOUND_IDS.open, 0.45, 1)
end
local function closeSound()
	playSound(SOUND_IDS.close, 0.45, 1)
end
...
task.spawn(function()
	while circle.Parent do
		local t = TweenService:Create(pulse, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
...
local function selectTab(name)
	for n, c in pairs(pages) do
		c.Visible = (n == name)
	end
	for n, b in pairs(tabButtons) do
		TweenService:Create(b, TweenInfo.new(0.2), {
			BackgroundColor3 = (n == name) and COLORS.accent or COLORS.card
		}):Play()
	end
end
...
local function makeToggle(parent, text, default, callback)
	...
	local function setState(s, silent)
		...
		TweenService:Create(ind, TweenInfo.new(0.2), {
			BackgroundColor3 = state and COLORS.accent or Color3.fromRGB(60, 60, 72)
		}):Play()
...
local function loadFlyGUI_V3()
	if flyGuiLoaded then
		return
	end
	flyGuiLoaded = true
	task.spawn(function()
		local ok, err = pcall(function()
			loadstring(game:HttpGet(FLY_GUI_V3_URL))()
		end)
		if not ok then
...
local function unloadFlyGUI_V3()
	...
	for _, g in ipairs(CoreGui:GetChildren()) do
		if g:IsA("ScreenGui") and g.Name ~= "MakaziMod" then
			...
			if n:find("fly") or n:find("gui") or n:find("xn") then
				g:Destroy()
			end
		end
	end
...
RunService.RenderStepped:Connect(function()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= player and p.Character then
			if aimTeamCheck and p.Team and p.Team == player.Team then
				continue
			end
			local char = p.Character
			if isAlive(char) then
				...
				if head then
					...
					if dist <= aimFOV then
						if aimVisibleOnly and not isVisible(head) then
							continue
						end
...
task.spawn(function()
	while task.wait(2) do
		if fpsBoostEnabled then
			...
			for _, obj in ipairs(fpsBoostedObjects) do
				pcall(function()
					obj.Enabled = true
				end)
			end
...
makeSlider(tabMain, "Задержка между ударами", 1, 30, 15, function(v)
	killDelay = v / 100
end)
...
makeSlider(tabVisual, "Плавность наведения", 1, 20, 4, function(v)
	aimSmooth = 0.9 / v
end)
makeSlider(tabVisual, "Дистанция наведения", 50, 2000, 500, function(v)
	aimFOV = v
end)
makeSlider(tabVisual, "Задержка смены цели", 1, 20, 5, function(v)
	aimSwitchDelay = v / 10
end)
...
makeSlider(tabVisual, "Размер хитбокса", 2, 20, 5, function(v)
	hitboxSize = v
end)
...
makeToggle(tabVisual, "☀  Яркое освещение", false, function(s)
	...
	if s then
		applyFullbright();
		notify("Fullbright вкл", true)
	else
		removeFullbright();
		notify("Fullbright выкл", false)
	end
end)
		...
		if hrp then
			...
			auraConn = RunService.RenderStepped:Connect(function(dt)
				...
				if auraParts[1] then
					...
					auraParts[1].Size = Vector3.new(0.2, 10 * pulse, 10 * pulse)
				end
				if auraParts[2] then
					auraParts[2].CFrame = CFrame.new(diskPos) * CFrame.Angles(0, 0, math.rad(90))
					auraParts[2].Size = Vector3.new(0.25, 11 * pulse, 11 * pulse)
				end
				local dotRadius = 5.2 * pulse
				local count = 0
				for i = 3, 10 do
...
RunService.RenderStepped:Connect(function()
	...
	local camera = workspace.CurrentCamera
	local myChar = player.Character
	if not myChar or not camera then
		return
	end
...
makeSlider(tabGames, "Дистанция захвата", 50, 1500, 500, function(v)
	autoWinRange = v
end)
makeSlider(tabGames, "Скорость стрельбы", 1, 20, 10, function(v)
	autoWinFireRate = v / 100
end)
makeSlider(tabGames, "Хедшот-биас (%)", 0, 100, 85, function(v)
	autoWinHeadshotBias = v / 100
end)
...
makeSlider(tabFun, "Размер PP", 1, 5, 2, function(v)
	_G.MakaziPPSize = v
end)
...
makeSlider(tabFun, "Скорость спина (°/сек)", 180, 2160, 720, function(v)
	spinSpeed = v
	if spinAV then
		spinAV.AngularVelocity = Vector3.new(0, math.rad(v), 0)
	end
end)
...
local function flingPlayer(target)
	...
	if not myHRP or not tHRP or not tHum or tHum.Health <= 0 then
		return false
	end
...
makeSlider(tabFun, "Дистанция флинга", 5, 40, 12, function(v)
	flingRange = v
end)
makeSlider(tabFun, "Сила флинга", 100, 2000, 500, function(v)
	flingPower = v
end)
task.spawn(function()
	while task.wait(0.2) do
			...
			if myHRP then
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= player and p.Character then
						...
						if hrp and hum and hum.Health > 0 then
							local d = (hrp.Position - myHRP.Position).Magnitude
							if d < flingRange and d > 0.5 then
								flingPlayer(p)
							end
						end
					end
...
makeToggle(tabFun, "⛵  Построить корабль", false, function(s)
	if s then
		...
		RunService.Heartbeat:Connect(function()
			if not boatEnabled or not boatVel or not boatVel.Parent or not boatHull then
				return
			end
			local camera = workspace.CurrentCamera
...
makeSlider(tabFun, "Скорость корабля", 20, 400, 120, function(v)
	boatSpeed = v
end)
makeSlider(tabFun, "Высота корабля", 2, 60, 8, function(v)
	boatHeight = v
end)
...
makeButton(tabMisc, "❌  Выключить всё", function()
	if spinEnabled then
		...
		if h and spinSavedAutoRotate ~= nil then
			h.AutoRotate = spinSavedAutoRotate
		end
	end
	if auraEnabled then
		auraEnabled = false
		...
		if _G.MakaziSahur and not sahurModel then
			...
			if hrp then
				...
				local ok, model = pcall(function()
					return InsertService:LoadAsset(SAHUR_ID)
				end)
				if not ok or not model then
					...
					for _, d in ipairs(model:GetDescendants()) do
						if d:IsA("BasePart") then
							...
							d.Anchored = false
							d.Massless = true
							if not primary or d.Size.Magnitude > primary.Size.Magnitude then
								primary = d
							end
						end
					end
		...
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if moved then
				local newPos = UDim2.new(
                    startPos.X.Scale, startPos.X.Offset + d.X, startPos.Y.Scale, startPos.Y.Offset + d.Y)
...
task.delay(1, function()
	notify("Makazi Mod v10 загружен ✅", true)
	print("[MAKAZI] Всё готово! v10 FULL")
end)
