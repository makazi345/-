...
task.spawn(function()
    while circle.Parent do
        local t = TweenService:Create(pulse, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
            { Size = UDim2.new(1.4, 0, 1.4, 0), Position = UDim2.new(-0.2, 0, -0.2, 0), BackgroundTransparency = 0.95 })
        t:Play()
        t.Completed:Wait()
    end
end)
...
local function makeButton(parent, text, callback)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.15), { BackgroundColor3 = COLORS.cardHover }):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.15), { BackgroundColor3 = COLORS.card }):Play()
    end)
...
RunService.RenderStepped:Connect(function()
    ...
    if target then
        local stillAlive = target.char and target.char.Parent
            and target.char:FindFirstChildOfClass("Humanoid")
            and target.char:FindFirstChildOfClass("Humanoid").Health > 0
...
makeToggle(tabFun, "⛵  Построить корабль", false, function(s)
    if s then
        ...
        if boatModel then pcall(function() boatModel:Destroy() end) end
        if boatVel then boatVel:Destroy() end
        if boatPos then boatPos:Destroy() end
        if boatAlign then boatAlign:Destroy() end
        if boatAttach then boatAttach:Destroy() end
        boatModel = nil
...
makeSlider(tabFun, "Скорость корабля", 20, 400, 120, function(v) boatSpeed = v end)
makeSlider(tabFun, "Высота корабля", 2, 60, 8, function(v) boatHeight = v end)
